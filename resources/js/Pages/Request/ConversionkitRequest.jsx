...
export default function ConversionkitRequest({ tableData, tableFilters, packagesFrom, packagesTo, empData, emp_data, conversionkit_request }) {
...
const closeModals = () => {
  ...
  setSelectedRow(null);
    ...
    const getStatusBadge = (status) => {
        ...
        switch (status) {
            case "For Approval":
                badgeColor = "text-blue-600 bg-blue-100 border-blue-700 hover:bg-blue-800 hover:text-white hover:border-white ";
                break;
            case "For Acknowledge":
                badgeColor = "text-teal-600 bg-teal-100 border-teal-700 hover:bg-teal-800 hover:text-white hover:border-white";
                break;
        }
...
const tableRows = tableData.data.map((row) => ({
  ...
  actions: (() => {
    ...
    return (
      <p className={`${currentColor} text-sm flex items-center transition-colors duration-500 text-sky-500`}>
